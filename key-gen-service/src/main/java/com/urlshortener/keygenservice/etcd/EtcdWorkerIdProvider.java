...
public class EtcdWorkerIdProvider {
    ...
    @Value("${app.key-gen.etcd.max-worker-id:255}")
    ...
    void init() throws Exception {
        ...
        keepAlive = leaseClient.keepAlive(leaseId, new StreamObserver<>() {
            ...
            public void onError(Throwable t) {
                log.error("etcd keepalive error", t);
            }

            @Override
            public void onCompleted() {
                log.info("etcd keepalive stream closed");
            }
        });
    }
    ...
    public int getWorkerId() {
        if (workerId < 0) {
            throw new IllegalStateException("Worker ID not yet assigned");
        }
        return workerId;
    }
